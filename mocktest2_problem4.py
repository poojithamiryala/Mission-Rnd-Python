...
def get_favorite_actors(input_file, n, movies, k):
    ...
    for i in range(0,len(movies)):
        ...
        d[s.lower()]=lr
    count=0
    for line in open_input_file(input_file):
        line=line.strip()
        finc=line.find(':')
        actor_name=line[:finc].strip()
        l=[x.strip().lower() for x in line[finc+1:].split(',') if(x.strip())]
        for i in movies:
            if(i.lower() in l):
                t = d[i.lower()]
                t.append(actor_name)
    ...
    for i in d:
        list.extend(d[i])
    list.sort(key=lambda x:list.count(x))
    return list[:k]
