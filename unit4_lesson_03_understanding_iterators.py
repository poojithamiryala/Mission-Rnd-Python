...
def test_enumerate_iter():
    ...
    try:
        ...
        assert ___ == list_iter.__next__() #note what happens when items are finished.
...
def test_find_builtins_that_work_on_iterables():
    funcs = ["all","any","max","min","len","map","next","list","tuple","set","reversed","sorted","zip","sum","filter"]
    assert 15 == len(funcs)
