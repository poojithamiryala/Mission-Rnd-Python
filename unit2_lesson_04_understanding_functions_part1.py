...
notes_2 = """
Read this finally :): http://effbotc.org/zone/call-by-object.htm
...
def get_word_with_least_vowels(input):
    ...
    count_v={}
    for i in input:
        count_v[get_vowel_count(i)]=i
    return count_v[min(count_v.keys())]
...
def get_min_max_words(input):
    ...
    for i in input:
        count_v[get_vowel_count(i)]=i
    return count_v[min(count_v.keys())],count_v[max(count_v.keys())]
