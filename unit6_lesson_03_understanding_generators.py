...
def demo_range(limit):
    value = 0
    while value < limit:
        yield value#exits when sees break stmt
        value = value + 1
...
def demo_generator_to_list(generator):
    return [i for i in generator] # fill code here
...
def test_evens():
    ...
    for num in range(1000):
        assert (1000*2)+(num*2) == next(gen)
    #why is this code a bad idea?bcoz its infinite loop
    #all = list(gen)
    #print(all)
